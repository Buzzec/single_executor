...
use core::fmt;
use core::fmt::Debug;
use core::task::{Waker, RawWakerVTable, RawWaker};
use alloc::boxed::Box;
...
pub fn try_spawn_blocking<F, T, CS>(function: F) -> Result<(impl Future<Output = T> + 'static + Send, CS::ThreadHandle), CS::SpawnError>
where
...
pub fn spawn_blocking<F, T, CS>(function: F) -> (impl Future<Output = T> + 'static + Send, CS::ThreadHandle)
where
    F: FnOnce() -> T + Send + 'static,
    T: 'static + Send,
...
/// use single_executor::{AsyncExecutor, SleepFutureRunner, spawn_blocking};
...
///
/// let executor = AsyncExecutor::<_, StdThreadFunctions>::new(ParkQueue::<_, StdThreadFunctions>::default());
/// let sleep_runner = Rc::new(SleepFutureRunner::new(ParkQueue::<_, StdThreadFunctions>::default()));
///
    ...
    CS: ConcurrentSystem<()>,
{
    ...
    pub fn handle(&self) -> ExecutorHandle<Q>{
        ExecutorHandle{ queue: Arc::downgrade(&self.task_queue) }
    }
    ...
    pub fn local_handle(&self) -> LocalExecutorHandle<Q>{
        LocalExecutorHandle{ queue: Arc::downgrade(&self.task_queue), phantom_send_sync: Default::default() }
    }
    ...
    /// you.
    pub fn submit_loop<SQ, Func, Fut>(&self, mut future_func: Func, delay: Duration, sleep_runner: impl Deref<Target=SleepFutureRunner<SQ, CS>> + 'static)
    where
    ...
    pub fn run(&self, stop: impl Deref<Target=AtomicBool>) {
        ...
        while !stop.load(Ordering::Acquire) {
            ...
            if let Some(task) = task {
                ...
                if unsafe { task.poll(&waker) }.is_pending(){
                    self.task_queue.try_push(task).expect("Could not push task, queue full!");
                }
            }
            _run_iters += 1;
        }
...
impl From<WakerData> for RawWaker{
    fn from(from: WakerData) -> Self {
        RawWaker::new(
            Box::into_raw(Box::new(from)) as *const (),
            &WAKER_VTABLE,
        )
    }
}
static WAKER_VTABLE: RawWakerVTable = RawWakerVTable::new(
    ...
    |ptr| {
        ...
        data.task_queue.try_push(data.task.clone()).expect("Queue is full!");
    },
    |ptr| {
        let data = unsafe { Box::from_raw(ptr as *const WakerData as *mut WakerData) };
        drop(data);
...
impl<Q> ExecutorHandle<Q> where Q: 'static + TimeoutQueue<Item = AsyncTask> + Send + Sync{
    /// Submits a task to the executor this handle came from. Will return [`Err`] if the executor was dropped.
    pub fn submit<F>(&self, future: F) -> Result<(), F> where F: Future<Output=()> + 'static + Send{
        match self.queue.upgrade() {
            None => Err(future),
            Some(queue) => {
                queue.try_push(AsyncTask::new(future)).expect("Queue is full!");
                Ok(())
            }
        }
    }
...
impl<Q> LocalExecutorHandle<Q> where Q: 'static + TimeoutQueue<Item = AsyncTask> + Send + Sync{
    /// Submits a task to the executor this handle came from. Will return [`Err`] if the executor was dropped.
    pub fn submit<F>(&self, future: F) -> Result<(), F> where F: Future<Output=()> + 'static{
        match self.queue.upgrade() {
            None => Err(future),
            Some(queue) => {
                queue.try_push(AsyncTask::new(future)).expect("Queue is full!");
...
#[cfg(test)]
mod test{
    use crate::{AsyncExecutor, SleepFutureRunner, spawn_blocking, SleepMessage};
    use concurrency_traits::queue::{ParkQueue, TimeoutQueue};
    use concurrency_traits::StdThreadFunctions;
    use core::time::Duration;
    use alloc::rc::Rc;
    ...
    fn slam_test(){
        let executor = AsyncExecutor::<_, StdThreadFunctions>::new(ParkQueue::<_, StdThreadFunctions>::default());
        let sleep_runner = Rc::new(SleepFutureRunner::<ParkQueue<_, StdThreadFunctions>, StdThreadFunctions>::new(Default::default()));
        let loop_function = |atom_count: Rc<AtomicUsize>|{
            async move{
        ...
        for _ in 0..100{
            let atom_count = Rc::new(AtomicUsize::new(0));
            atom_counts.push(atom_count.clone());
            executor.submit_loop(move || {
                let atom_count = atom_count.clone();
                loop_function(atom_count)
            }, Duration::from_millis(100), sleep_runner.clone());
        }
        let stop = Arc::new(AtomicBool::new(false));
        let stop_clone = stop.clone();
        spawn(move || {
    ...
    fn doc_test(){
        ...
        /// Dummy function
        async fn get_something_from_io<Q>(sleep_runner: Rc<SleepFutureRunner<Q, StdThreadFunctions>>)
        where
            Q: 'static + TimeoutQueue<Item=SleepMessage<StdThreadFunctions>> + Send + Sync
        {
            sleep_runner.sleep_for(Duration::from_millis(500)).await;
        }
        executor.submit(get_something_from_io(sleep_runner));
        ...
        // executor.submit(async {
        //     spawn_blocking::<_, _, StdThreadFunctions>(block_for_a_while).0.await;
        //     // assert_eq!(spawn_blocking::<_, _, StdThreadFunctions>(block_for_a_while).0.await, 100);
        // });
